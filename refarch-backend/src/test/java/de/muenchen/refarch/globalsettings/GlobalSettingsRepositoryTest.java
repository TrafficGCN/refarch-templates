...
class GlobalSettingsRepositoryTest {
    ...
    void findAll_ShouldReturnSettings() {
        ...
        // Test findAll
        List<GlobalSettings> result = globalSettingsRepository.findAll();

        assertThat(result).hasSize(1);
        GlobalSettings foundSettings = result.get(0);
        assertThat(foundSettings.getWebsiteName()).isEqualTo("Test Website");
    ...
    void update_ShouldUpdateSettings() {
        ...
        GlobalSettings settings = new GlobalSettings();
        settings.setSessionDurationMinutes(480);
        settings.setWebsiteName("Initial Name");
        settings.setGlobalCommentsEnabled(true);
        settings.setMaintenanceMode(false);
        settings.setMaxUploadSizeMb(10);
        settings.setDefaultLanguage("en");
